...
```{r}
training <- createDataPartition(trainingData$classe, p = 0.7, list = FALSE)
trainingSet <- trainingData[training, ]
testingSet <- trainingData[-training, ]
dim(trainingSet)
dim(testingSet)
```
...
```{r}
nzvTraining <- nearZeroVar(trainingSet)
trainingSet <- trainingSet[, -nzvTraining]
testingSet <- testingSet[, -nzvTraining]
dim(trainingSet)
dim(testingSet)
```
...
testingSet <- testingSet[, naSet == FALSE]
dim(trainingSet)
dim(testingSet)
```
...
Have 0.9999 accuracy with this model.  Success!
